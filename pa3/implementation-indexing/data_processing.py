        ...
        for i in range(1, len(html_files)):
            ...
            for el in tokenized:
                ...
                if word not in stopwords.stop_words_slovene:
                    ...
                    if db.get_posting(word, site + "/" + html_files[i]) is None:
                        indexes = [str(j) for j, val in enumerate(tokenized) if val == el]
                        frequency = len(indexes)
                        db.insert_posting(word, site + "/" + html_files[i], frequency, ','.join(indexes))
                        print(word + " ||" + html_files[i] + " ||" + str(frequency) + " ||" + ','.join(indexes))
